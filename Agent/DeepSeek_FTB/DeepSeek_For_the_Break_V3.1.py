...
# -*- coding: utf-8 -*-
"""
DeepSeek_For_the_Break v3.1 — 全功能学术 AI 助手
==================================================
  v3.1 升级:
    🐛 修复:
      • PDF OCR 临时文件自动清理
      • LaTeX 编译增加 bibtex/biber 支持
      • arXiv HTTPS、DOI import re 移到顶部
      • pytesseract 健康检查改为实测验证
      • BibTexTool format APA/IEEE 语法错误修复
    🔧 重构:
      • _run_sandbox() 统一沙箱执行
      • SmartReadPathTool 拆分为独立函数
      • 文件编码缓存
      • agent_memory.db 大小上限自动瘦身
    🆕 新增:
      • PandocConvertTool   — Markdown/LaTeX → PDF/DOCX/HTML
      • PDFAnnotExtractTool — PDF 批注/高亮提取
      • AcademicTranslateTool — 学术中英互译
      • SemanticScholarTool — Semantic Scholar 检索
      • PresentationGenTool — Markdown 大纲 → PPTX
"""
...
_log("START", "═" * 50)
_log("START", "  DeepSeek_For_the_Break  v3.1  Academic AI")
_log("START", "═" * 50)
...
#  2. System Prompt
# ═══════════════════════════════════════════════════════════
SYSTEM_PROMPT = """你是 DeepSeek_For_the_Break v3.1，高度专业、严谨且全面的学术AI助手。

## 核心能力
- **文献检索**: ArXivSearchTool、SemanticScholarTool（含引用网络）、DOIMetadataTool、DuckDuckGoSearchTool
- **文档处理**: PDF/Word/PPT/Excel 读取（SmartReadPathTool）、PDF表格提取、PDF批注提取、OCR
- **学术写作**: LaTeX 创建编译（+bibtex）、Word/Markdown/PPTX 输出
...
## 行为准则
1. 客观严谨，结构清晰，主动承认局限性。
2. **【核心铁律】** SmartReadPathTool 是唯一读取本地文件的工具。
...
def BibTexTool(
    ...
    if mode == "parse":
        ...
        for m in pattern.finditer(raw):
            fields = {}
            for fm in re.finditer(r'(\w+)\s*=\s*[{"]([^}"]*)[}"]', m.group(3)):
                fields[fm.group(1).lower()] = fm.group(2).strip()
            entries.append({"type": m.group(1), "key": m.group(2).strip(), "fields": fields})
...
# ═══════════════════════════════════════════════════════════
#  🆕 v3.1 工具 20: Semantic Scholar 检索
# ═══════════════════════════════════════════════════════════
...
def PresentationGenTool(markdown_content: str, filename: str = "presentation",
    ...
    prs = Presentation()
    if theme == "dark":
        bg_color = RGBColor(0x1A, 0x1A, 0x2E); text_color = RGBColor(0xE0, 0xE0, 0xE0); accent_color = RGBColor(0x64, 0xFF, 0xDA)
    elif theme == "academic":
        bg_color = RGBColor(0xFF, 0xFF, 0xFF); text_color = RGBColor(0x33, 0x33, 0x33); accent_color = RGBColor(0x00, 0x55, 0xAA)
    else:
        bg_color = RGBColor(0xFF, 0xFF, 0xFF); text_color = RGBColor(0x33, 0x33, 0x33); accent_color = RGBColor(0x00, 0x7A, 0xCC)
    ...
    for slide_text in slides_text:
        ...
        for line in lines:
            line = line.strip()
            if line.startswith("# ") and not title: title = line[2:]
            elif line.startswith("## ") and not subtitle: subtitle = line[3:]
            elif line.startswith("- ") or line.startswith("* "): bullets.append(line[2:])
        if title:
            slide.shapes.title.text = title
            if slide.shapes.title.text_frame:
                for run in slide.shapes.title.text_frame.paragraphs[0].runs:
                    run.font.size = Pt(32); run.font.color.rgb = accent_color
        if subtitle and len(slide.placeholders) > 1:
            try:
                tf = slide.placeholders[1].text_frame; tf.clear()
                p = tf.paragraphs[0]; p.text = subtitle
                p.font.size = Pt(18); p.font.color.rgb = text_color
        ...
        if bullets and len(slide.placeholders) > 1:
            try:
                ...
                for b in bullets:
                    ...
                    p.font.size = Pt(16); p.font.color.rgb = text_color
            except Exception: pass
    prs.save(sp)
