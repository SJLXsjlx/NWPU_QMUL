...
# -*- coding: utf-8 -*-
"""
DeepSeek_For_the_Break v3.2 — 全功能学术 AI 助手 (HOTFIX#2)
============================================================
  v3.4-hotfix 修复:
    🐛 紧急修复:
      • [FIX#11] 数据库备份文件名加入时分秒 (%Y%m%d_%H%M%S)，
        修复同一天多次启动时 os.rename() 因目标文件已存在而抛出
        FileExistsError (WinError 183) 的 bug。
  v3.1-hotfix 修复:
    🐛 紧急修复:
      ...
      • [FIX#4] PresentationGenTool：主题 bg_color/text_color 正确应用
      • [FIX#5] _read_pdf_file OCR 清理：添加 try/finally
      • [FIX#6] PythonSandboxTool：工具描述明确安全警告
    🔧 改进:
...
_MAX_DB_SIZE_MB = 100
# ── [FIX#11] 备份文件名加入时分秒，防止同一天多次启动时报 FileExistsError ──
...
_log("START", "═" * 50)
_log("START", "  DeepSeek_For_the_Break  v3.2-hotfix  Academic AI")
_log("START", "═" * 50)
...
#  2. System Prompt
# ═══════════════════════════════════════════════════════════
SYSTEM_PROMPT = """你是高度专业、严谨且全面的学术AI助手。
...
def PythonSandboxTool(code: str, timeout: int = 30) -> str:
    """执行 Python 代码。⚠ 安全警告：并非真正隔离的沙箱，请勿执行不可信代码。"""
    if timeout > 120: timeout = 120
    try:
...
HELP_TEXT = f"""
{_C.BOLD}╔══════════════════ v3.2-hotfix 工具一览 (21个) ═══════════════════╗{_C.W}
