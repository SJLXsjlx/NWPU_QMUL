...
# -*- coding: utf-8 -*-
"""
DeepSeek_For_the_Break v3.1 — 全功能学术 AI 助手 (HOTFIX)
==========================================================
  v3.1-hotfix 修复:
    🐛 紧急修复:
      • [FIX#1] BibTexTool IEEE 作者格式：修复 "., ".join 错误，
        改为标准 "F. Lastname, F. Lastname, and F. Lastname" 格式
      • [FIX#2] BibTexTool 解析器重写：引入 bibtexparser 库（可选），
        并提供带嵌套括号匹配状态机的回退解析器，正确处理复杂 .bib
      • [FIX#3] PandocConvertTool：移除硬编码 SimSun，自动检测平台字体；
        支持 CJKmainfont；允许 extra_args 覆盖
      • [FIX#4] PresentationGenTool：主题 bg_color/text_color
        正确应用到幻灯片背景和正文文字（此前仅 accent_color 生效）
      • [FIX#5] _read_pdf_file OCR 清理：添加 try/finally，
        确保 OCR 异常时临时 PNG 文件被删除
      • [FIX#6] PythonSandboxTool：工具描述明确声明
        "⚠ 注意：非真正安全沙箱，不应执行不可信代码"
    🔧 改进:
      • CompileLatexTool：bibtex 运行前检查 .bib 文件是否存在
      • PresentationGenTool：slide_layouts/placeholders 索引安全回退
      • AcademicTranslateTool：ChatOpenAI 导入提升至模块顶层
      • SYSTEM_PROMPT：澄清 SmartReadPathTool 是"通用"读取工具，
        专用工具（PDFTableExtract 等）可直接操作文件
"""
...
from langchain_core.messages import HumanMessage  # [FIX#10] 提升至顶层
from langgraph.prebuilt import create_react_agent
from langgraph.checkpoint.sqlite import SqliteSaver
from pptx import Presentation
from pptx.util import Inches as PptInches
from pptx.oxml.ns import qn  # 用于设置幻灯片背景
...
_DETECTED_CJK_FONT = _detect_cjk_font()  # [FIX#3]
...
_health: Dict[str, bool] = {
    ...
    "API Key (.env)":         True,
    "bibtexparser":           _BIBTEXPARSER_AVAILABLE,  # [FIX#2]
    "CJK Font (detected)":    bool(_DETECTED_CJK_FONT),  # [FIX#3]
}

_log("START", "═" * 50)
_log("START", "  DeepSeek_For_the_Break  v3.1-hotfix  Academic AI")
_log("START", "═" * 50)
...
# ═══════════════════════════════════════════════════════════
#  2. System Prompt  [FIX#7] 澄清工具分工
# ═══════════════════════════════════════════════════════════
SYSTEM_PROMPT = """你是 DeepSeek_For_the_Break v3.1-hotfix，高度专业、严谨且全面的学术AI助手。
...
def _read_pdf_file(fp: str) -> str:
    """读取 PDF 文件，优先提取文本；无文本层时回退 OCR。
    [FIX#5] OCR 临时文件清理使用 try/finally 保护。"""
    try:
        ...
        for i, page in enumerate(doc):
            ...
            pix.save(tmp)
            # [FIX#5] try/finally 确保 OCR 异常时也清理临时文件
            try:
...
# ═══════════════════════════════════════════════════════════
#  [FIX#2] 健壮 BibTeX 解析器（回退用）
# ═══════════════════════════════════════════════════════════

def _find_matching_brace(s: str, start: int) -> int:
    """从 start（指向 '{'）找到匹配的 '}' 位置。返回索引或 -1。"""
    depth = 1
    i = start + 1
...
def _parse_field_assignment(segment: str) -> Tuple[str, str]:
    """解析单个 'fieldname = value' 片段。返回 (field_name_lower, value)。"""
    eq_pos = segment.find('=')
    if eq_pos == -1:
        return "", ""
...
def _robust_parse_bibtex(raw: str) -> List[Dict[str, Any]]:
    """不使用 bibtexparser 时的回退解析器，支持嵌套括号。"""
    entries = []
    i = 0
    while i < len(raw):
        at_pos = raw.find('@', i)
        if at_pos == -1:
            break
        # 确保是 BibTeX 条目类型（后面跟字母）
        j = at_pos + 1
        while j < len(raw) and raw[j].isalpha():
            j += 1
        if j == at_pos + 1:  # 不是字母，跳过
            i = j + 1
            continue
...
def BibTexTool(
    ...
    if mode == "format":
        ...
        if citation_style == "apa":
            ...
            # [FIX#1] 修复 IEEE 作者格式: "F. Lastname, F. Lastname, and F. Lastname"
            ieee_authors = []
            for au in au_list:
                ...
                if len(parts) >= 2:
                    ...
                    ieee_authors.append(parts[0])
                else:
                    ieee_authors.append(au)

            if len(ieee_authors) == 1:
                au_str = ieee_authors[0]
            elif len(ieee_authors) == 2:
                au_str = f"{ieee_authors[0]} and {ieee_authors[1]}"
            elif len(ieee_authors) <= 6:
                au_str = ", ".join(ieee_authors[:-1]) + f", and {ieee_authors[-1]}"
            else:
                au_str = ", ".join(ieee_authors[:6]) + ", et al."
...
# ═══════════════════════════════════════════════════════════
#  🆕 工具 17: Pandoc 格式转换  [FIX#3] 移除硬编码 SimSun
# ═══════════════════════════════════════════════════════════
...
def PandocConvertTool(source_path: str, output_format: str = "pdf",
    ...
    # [FIX#3] PDF 转换: 自动检测中文字体，允许 extra_args 覆盖
    if output_format == "pdf" and "--pdf-engine" not in extra_args:
        xelatex = shutil.which("xelatex")
        if xelatex:
            cmd.extend(["--pdf-engine=xelatex"])
            # 仅在用户未通过 extra_args 指定字体时自动检测
            if "-V" not in extra_args and "mainfont" not in extra_args.lower():
                if _DETECTED_CJK_FONT:
                    ...
                    _log("INFO", f"Pandoc PDF: 自动检测字体 {_DETECTED_CJK_FONT}")
                else:
                    # 不指定字体，让 xelatex 使用默认
...
def AcademicTranslateTool(text: str, direction: str = "zh2en",
    ...
    """学术中英互译，保留术语一致性。
    [FIX#10] ChatOpenAI/HumanMessage 已提升至模块顶层，避免每次调用重新导入。"""
...
# ═══════════════════════════════════════════════════════════
#  🆕 工具 21: Markdown → PPTX 演示生成  [FIX#4][FIX#9]
# ═══════════════════════════════════════════════════════════
...
def PresentationGenTool(markdown_content: str, filename: str = "presentation",
    ...
    # [FIX#9] 安全获取 placeholders
    def _get_placeholder(slide, idx=1):
        """安全获取 placeholder，不存在则返回 None。"""
        try:
    ...
    for slide_text in slides_text:
        ...
        if body_shape:
            ...
            if subtitle:
                ...
                p.text = subtitle
                p.font.size = Pt(18)
                p.font.color.rgb = text_color  # [FIX#4]
...
HELP_TEXT = f"""
{_C.BOLD}╔══════════════════ v3.1-hotfix 工具一览 (21个) ═══════════════════╗{_C.W}
