...
# -*- coding: utf-8 -*-
"""
DeepSeek_For_the_Break v4.0 — 全功能学术 AI 助手 (MAJOR UPGRADE)
================================================================
  v4.0 重大升级:
    🚀 新增工具:
      • StudyPlanTool — 个性化学习计划生成
      • FlashcardTool — Anki/Quizlet 闪卡生成
      • KnowledgeGraphTool — 知识图谱 (Mermaid/Graphviz)
      • NoteOrganizerTool — 智能笔记整理与摘要
      • CodeReviewTool — 代码审查与优化建议
      • MathRenderTool — LaTeX 数学公式渲染
      • CitationNetworkTool — 引用网络分析
      • ProjectScaffoldTool — 项目脚手架生成
    🔧 大幅增强:
      • PresentationGenTool — 多主题/多布局/图表/图片/表格全支持
      • PythonSandboxTool — 增强沙箱/多文件/代码模板/复杂度分析
      • ChartGenerationTool — 更多图表类型/seaborn支持
      • EditWordDocTool — 表格/样式/目录支持
    🐛 修复:
      • [FIX#12] duckduckgo_search → ddgs 迁移，向后兼容
      • [FIX#13] PythonSandbox 错误行号映射
      • [FIX#14] 所有工具的超时与异常处理增强
      • [FIX#15] 文件路径跨平台兼容优化
"""
    ...
    if sys.platform == "win32":
        for font in ["SimSun", "SimHei", "Microsoft YaHei", "KaiTi", "FangSong"]:
            try:
                ...
                if result.stdout.strip(): return font
...
_log("START", "═" * 60)
_log("START", "  DeepSeek_For_the_Break  v4.0  Academic AI")
_log("START", "═" * 60)
...
# ═══════════════════════════════════════════════════════════
SYSTEM_PROMPT = """你是 DeepSeek_For_the_Break v4.0，高度专业、严谨且全面的学术AI助手。
...
# 思考模式：DeepSeek 思考模式可能与 tool_calls 存在兼容性问题
# 设置环境变量 DFTB_THINKING_MODE=enabled 来启用（默认关闭）
...
def ProjectScaffoldTool(project_name: str, project_type: str = "python",
    ...
    # safe package name: Python packages only allow lowercase letters/digits/underscore, cannot start with digit
    safe_name = re.sub(r'[^a-zA-Z0-9_]', '_', project_name)
    if safe_name and safe_name[0].isdigit():
        safe_name = 'pkg_' + safe_name  # package name cannot start with digit
    safe_name = safe_name.lower()  # Python package convention: all lowercase
...
# -- 自定义 Agent 图 (替换 create_react_agent) --
# 使用显式的 should_continue 路由，严格基于 isinstance 和 tool_calls 判断，
# 避免 DeepSeek 思考模式或老版本 function_call 格式导致的路由错误。
class _AgentState(TypedDict):
    messages: Annotated[list, add_messages]

def _call_model(state: _AgentState) -> dict:
    messages = state["messages"]
    # [v4.0.1] 验证消息完整性：检查是否有孤立的 tool_calls（无对应 ToolMessage）
    # DeepSeek API 要求 tool_calls 和 tool 响应必须紧密相邻
    _validated = []
...
def _should_continue(state: _AgentState) -> str:
    ...
    # 严格使用 isinstance 和 tool_calls 属性判断
    ...
    try:
        ...
        for event in agent.stream(inputs, config, stream_mode="updates"):
                if "agent" in event:
                    ...
                    for msg in messages:
                        # 处理 DeepSeek 思考内容 (reasoning_content)
                        ...
                        # 处理文本内容 (使用 isinstance 严格判断)
                        if isinstance(msg, AIMessage) and msg.content:
                            elapsed = time.time() - t_start
                            print(f"{_C.G}🤖 DeepSeek{_C.W} ({elapsed:.1f}s):\n{msg.content}\n")

                        # 处理工具调用 (使用 isinstance 严格判断，独立 if 而非 elif)
